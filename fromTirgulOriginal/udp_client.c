...
int main() {
    ...
    // send the message
    int sendResult = sendto(s, message, messageLen, 0, (struct sockaddr *)&serverAddress, sizeof(serverAddress));
    if (sendResult == -1) {
    ...
    int recvLen = recvfrom(s, bufferReply, sizeof(bufferReply) - 1, 0, (struct sockaddr *)&fromAddress, &fromAddressSize);
    if (recvLen == -1) {
