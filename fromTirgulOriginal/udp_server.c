...
int main() {
    ...
    // Bind
    int bindResult = bind(s, (struct sockaddr *)&serverAddress, sizeof(serverAddress));
    if (bindResult == -1) {
    ...
    while (1) {
        ...
        recv_len = recvfrom(s, buffer, sizeof(buffer) - 1, 0, (struct sockaddr *)&clientAddress, &clientAddressLen);
        if (recv_len == -1) {
        ...
        int sendResult = sendto(s, message, messageLen, 0, (struct sockaddr *)&clientAddress, clientAddressLen);
        if (sendResult == -1) {
